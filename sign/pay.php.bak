...
//增加vip天数
function addVipDays($user_id,$days){
  	 /*
     *  bmobObject 的例子
     */
    $cond = array("email"=>$user_id);
    $condition_str = json_encode($cond);
    //echo $condition_str;
    $bmobObj = new BmobObject("RoleMember");
    $res=$bmobObj->get("",array('where='.$condition_str,'limit=2'));
    $roleMember = current((Array)$res)[0];

    //订单vip信息存在的话，更新vip信息，否则vip会员信息
    if($roleMember != null)
    { 
    	 $server_time = msectime();
         $vip_oldtime = $roleMember->expiredAt;      
         $is_disabled = $roleMember->isDisabled;
         
         if($server_time > $vip_oldtime)
         {
         	 $vip_oldtime = $server_time;
         }
      
        
         //毫秒数
         $vip_newtime = $vip_oldtime + $days * 24 * 3600 * 1000;
         $vip_date = date('Y-m-d H:i:s',$vip_newtime/1000);
      
         $bmobObj->update($roleMember->objectId, array("isDisabled"=>(bool)$is_disabled,"expiredAt"=>$vip_newtime,"expiredStr"=>$vip_date)); 
    }
  	else
    {
         $vip_oldtime = msectime();
         //毫秒数
         $vip_newtime = $vip_oldtime + $days * 24 * 3600 * 1000;
         $vip_date = date('Y-m-d H:i:s',$vip_newtime/1000); 
      
    	 $bmobObj->create(array("isDisabled"=>false,"email"=>$user_id,"type"=>'支付会员',"expiredAt"=>$vip_newtime,"expiredStr"=>$vip_date)); //添加对象
    }
}
...
try {
    ...
    if(strtolower(md5($order_id.$user_id.$amount.'FCAB7B8F3358CBCED90F47F9C9E394F2')) != strtolower($sign))
    {
        echo 'invalid sign';
        return;
    }
  
    $sign_calc = md5($order_id.$user_id.$amount.'FCAB7B8F3358CBCED90F47F9C9E394F2');
     ...
     *  bmobObject 的例子
     */
    $cond = array("orderID"=>$order_id,"email"=>$user_id);
    $condition_str = json_encode($cond);
    //echo $condition_str;
    $bmobObj = new BmobObject("PayLog");
    $res=$bmobObj->get("",array('where='.$condition_str,'limit=2'));
    $firstOrder = current((Array)$res);

    //订单存在的话，更新订单信息，否则创建订单
    if($firstOrder != null)
    {
         $bmobObj->update($firstOrder[0]->objectId, array("serverCount"=>(float)$amount,"tradeID"=>$trade_id,"payTime"=>$pay_time,"result" =>$result_string,"sign" => $sign, "signCalc" => $sign_calc)); 
         if($firstOrder[0]->vipAddLocal === "success")
         {
         	 echo "success";
             return;
         }
         else if($firstOrder[0]->vipAddLocal === "wait")
         {
               //更新会员信息
         	  if($amount > 45)
              {
    ...
    {
    	$bmobObj->create(array("serverCount"=>(float)$amount,"tradeID"=>$trade_id,"payTime"=>$pay_time,"result" =>$result_string,"orderID" =>$order_id,"email" =>$user_id,"sign" => $sign, "signCalc" => $sign_calc, "vipAddLocal" => 'wait')); //添加对象
